...
- 统一的 CRUD 查询入口：
  ...
  - Redis / Valkey 使用原生命令。
- 懒加载数据库对象树。
- PostgreSQL、MySQL / MariaDB、SQLite 表数据浏览与安全编辑：
  - 双击表或视图打开服务端分页数据，每次加载执行精确 `COUNT`。
  - 结构化筛选、原始 `WHERE` / `ORDER BY` 片段和稳定排序。
  - 保留 `NULL`、文本和二进制值差异，暂存新增、更新和删除。
  - 参数化 SQL 与有序参数预览；确认后在单个事务内原子提交。
  - 使用非空主键/唯一键定位行，并以原始值进行乐观并发检查。
  - 简单单表 `SELECT` 结果可编辑；JOIN、CTE、聚合等结果明确降级为只读。
...
关系型数据库可双击对象树中的表或视图进入数据浏览；视图、没有稳定键的表以及复杂查询结果
保持只读。复杂的 SQL 执行计划图暂时使用可滚动 JSON/文本展示。
...
详细设计与取舍见
[ADR-0001：模块化数据库驱动架构](docs/adr/0001-modular-driver-architecture.md)和
...
表数据浏览使用独立的服务端分页路径，不受交互查询的本地缓冲分页限制。每页加载会分别执行
参数化数据查询和精确计数查询；默认按稳定键排序。编辑暂存在客户端，切换页、重载、执行新查询、
切换驱动或断开连接前必须先应用或放弃变更。
...
- 表数据编辑仅在存在非空主键或非空唯一键时启用。提交时更新/删除必须恰好影响一行，否则整批
  事务回滚并报告冲突。
- 原始 `WHERE` / `ORDER BY` 会按当前数据库方言验证为单个片段，但仍由数据库执行；只应输入
  自己信任的表达式。结构化筛选和所有数据变更值始终使用绑定参数。
